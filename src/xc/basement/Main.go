...
切片唯一合法的比较操作时和nil比较。一个nil值的切片并没有底层数组，一个nil值的切片的长度和容量都是0。但是我们不能说一个
长度和容量都是0的切片一定是nil。
例如 s2 := []int{}  s2 == nil 为false尽管他容量长度都为0
所以要判断一个切片是否为空，要用len(s) == 0来判断
*/
func sliceFunc() {
	var s []int
	fmt.Print(s)
	//引用类型不支持直接比较，只能和nil比较
	fmt.Print(s == nil)
	var b = []int{}
	fmt.Print(b == nil) // false
	//切片也能部分初始化
	var c = []int{1, 2}
	...
	//注：这种数组切片的使用方法与python的数组切片方法是一致的
	...
	//切片之后也可以再切片
	var subDD1 = subD[1:2]
	fmt.Println(subD, subD2, subD3, subDD1)

	//切片的容量是指从切片头指针，一直到最后元素的个数
	// 输出的是5， 6，因为底层数组是d，，且头指针指向的d数组的头部，而d的长度是6
	fmt.Println(len(subD), cap(subD))
	//而subD3 的容量为4，因为从切片头指针开始，到最后只有4个元素了，所以输出的是4,4
	fmt.Println(len(subD3), cap(subD3))

	//注意，切片是引用，当底层数组(原始数组)的数据发生变化，切片的数据也会发生变化

	//动态构建切片，之前的切片创建方式都是写死的，使用make函数可以动态的构造一个切片
	//make([]T, size, cap) 分别是：切片数据类型，切片中元素的数量，切片的容量
	a := make([]int, 2, 10)
	fmt.Print(a)
}
